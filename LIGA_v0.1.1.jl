...
type kMultvec
	...
	kMultvec(n::Int) = new(Vector{kb}(n))
end
#########################################################
"""
```
...
```
Receives as input parameters basic elements of the same type and returns the sum of them.

It can be used the operator " + " instead of call the function.
"""
function bsum(a::kb, b::kb)
	l = length(a.e)
	e = Vector{Bool}(l)
	fill!(e, false)
	sume = kb(e, 0)
...
```
Returns the reverse of the basis blade a or blade A.
"""
function breverse(a::kb)
	k = grade(a)
	e = copy(a.e)
	b = kb(e)
...
```
Returns the dual element of the basis blade a.
"""
function bdual(a::kb)
	l = length(a.e)
	e = Vector{Bool}(l)
...
```
Receives as input parameters multivectors of the same type and returns the sum of them.
...
```
Returns the dual element of the multivetor X or blade A.
"""
...
function scalar(A::kMultvec, B::kMultvec)
	...
	for i=1:l
		for j=1:m
			AB[i,j] = scalar(A.comp[i], B.comp[j])
		end
	end
	p = kMultvec(reshape(AB,l*m))
	p = reduct(p)
...
```
Returns the magnitude of the blade A.
"""
function bnorm(A::kblade)
	X = scalar(A, breverse(A))
	return sqrt(X)
end
#########################################################
